...
public class RecipeListItemAdapter extends BaseAdapter {
	...
	public void addRecipeItems(JSONObject jsonResult) throws JSONException,
			IOException {
		...
		for (int i = 0; i < matches.length(); i++) {
			...
			if (eachMatch.getJSONObject("attributes").length() > 0) {
				try {
					if (eachMatch.getJSONObject("attributes").getJSONArray(
							"course") != null)
						category = (String) eachMatch
								.getJSONObject("attributes")
								.getJSONArray("course").get(0);
				} catch (Exception e) {
				}
				;
			}
			...
			Recipe tmpRecipe = new Recipe(imgUrl, recipeName, category,
					description, ingredients, null);
		...
		Collections.sort(this.recipes, new Comparator<Recipe>() {
			...
			public int compare(Recipe recipe, Recipe recipe2) {
				return recipe.getCategory().compareToIgnoreCase(
						recipe2.getCategory());
			}
		});
		...
		for (int i = 0; i < this.recipes.size(); i++) {
			if (i == 0
					|| !this.recipes
							.get(i - 1)
							.getCategory()
							.equalsIgnoreCase(this.recipes.get(i).getCategory()))
				this.recipes.add(i, new Recipe(this.recipes.get(i)
						.getCategory()));
		}
	...
	public void expandCategory(String category) {
		for (int i = 0; i < RecipeListItemAdapter.this.recipes.size(); i++) {
			if (!this.recipes.get(i).getCategory().equalsIgnoreCase(category)) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = inflater.inflate(
					type == 1 ? R.layout.listitem_separator
							: R.layout.listitem_recipe, parent, false);
		}
		...
		if (type == 1) {
			...
			ImageView img = (ImageView) convertView
					.findViewById(R.id.recipeListItemImage);
